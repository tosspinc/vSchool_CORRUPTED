function getData() {
  ...
  .then(response => {                                               /*declares function*/
    ...
    const completedActionTrue = {                                   /*declares completedActionTrue as a variable.*/ 
      completed: true                                               /*verifies that object is true.*/
    };
    ...
    completeButton.type = "checkbox";                               /*sets completeButtons as a checkbox.*/
      ...
      if (`completeButton${response.data[i]._id}.checked`) {        /*if statement verifies that id is marked as checked.*/
         axios.put(`https://api.vschool.io/arnoldjones/todo/${id}`, completedActionTrue) /*adds data entry to my vschool database.*/
            .then(response => {                                   
                ...
                window.location.reload();                         /*refreshes the browser with the new data.*/
            })
    ...
    const deleteRequest = function(id) {                            /*declares deleteReques as a variable and assigns id function data to it.*/
      axios.delete(`https://api.vschool.io/arnoldjones/todo/${id}`)
        .then(response => {
            getData(),                                              /*calls the getData function.*/
            window.location.reload();                               /*refreshes the webpage with the data pulled from my vschool database file.*/
        })
