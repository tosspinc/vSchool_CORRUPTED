...
function showLocation(position){                                        /* declares showLocation as a function and assings position data to showLocation. */
    let latitude = position.coords.latitude;                          /* declares lattitude as a variable and assigns the lattitude coordinates to it. */
    let longitude = position.coords.longitude;                          /* declares longitude as a variable and assigns the longitude coordinates to it. */
    let url = "https://nominatim.openstreetmap.org/reverse?format=jsonv2lat=" + latitude + "&lon=" + longitude;
...
if (navigator.geolocation) {                                            /* if statement that allows the navigation function to work and assings geolocation data to it. */
    navigator.geolocation.getCurrentPosition(showLocation, showError);  /* assigns the current geolocation data to getCurrentPosition which is passed to geolocation. */
    } else {
...
function sendForm() {                                               /* declares the sendForm as a function. */
    ...
    window.open(`mailto:tosspinc@gmail.com?subject=${name}&body=${body}`);
};
